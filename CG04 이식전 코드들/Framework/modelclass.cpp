...
bool ModelClass::Initialize(ID3D11Device* device, const WCHAR* modelFilename, const WCHAR* textureFilename, float x)
{
	bool result;
	...
	result = InitializeBuffers(device, x);
	if(!result)
	{
		return false;
	}
...
bool ModelClass::InitializeBuffers(ID3D11Device* device, float x1)
{
	VertexType* vertices;
	...
	{
		vertices[i].position = XMFLOAT3(m_model[i].x + x1, m_model[i].y, m_model[i].z);
