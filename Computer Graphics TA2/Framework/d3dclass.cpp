...
bool D3DClass::InitializeGraphicsHW(int screenWidth, int screenHeight, bool vsync)
{
	...
	// Update the wcstombs_s call to match the correct parameter types
    error = wcstombs_s(&stringLength, m_videoCardDescription, sizeof(m_videoCardDescription), adapterDesc.Description, 128);
    if (error != 0)
    {
       return false;
    }
...
bool D3DClass::InitializeRasterizer()
{
	...
	rasterDesc.CullMode = GetCullMode();
	rasterDesc.DepthBias = 0;
	rasterDesc.DepthBiasClamp = 0.0f;
	rasterDesc.DepthClipEnable = true;
	rasterDesc.FillMode = GetFILLMode();
...
void D3DClass::BeginScene(float red, float green, float blue, float alpha, float brightness)
{
	float color[4];


	// Setup the color to clear the buffer to.
	color[0] = red * brightness;
	color[1] = green * brightness;
	color[2] = blue * brightness;
	color[3] = alpha * brightness;
