	...
	const WCHAR *fileName)
{
	...
	// Setup the description of the dynamic brightness constant buffer that is in the pixel shader.
	...
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, float brightness)
{
	...
	BrightnessBufferType* brightnessdataPtr;
	...
	// 밝기 값 업데이트
	D3D11_MAPPED_SUBRESOURCE brightnessmappedResource;
	HRESULT brightnessresult = deviceContext->Map(m_brightnessBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &brightnessmappedResource);
	if (FAILED(brightnessresult))
	{
		return false;
	}

	brightnessdataPtr = (BrightnessBufferType*)brightnessmappedResource.pData;
	brightnessdataPtr->brightness = brightness;
