...
///////////////**************new**************////////////////////
// The Sky Geometry
// This function will create an index and vertex buffer we will use to map our sky to. This function uses Vectors, 
// which are dynamic arrays, so we must include the vector header. Our skybox will also use a separate vertex and 
// pixel shader from our other geometry, so we need to declare those here. Since we are loading in a texture we will 
// pass to the shader to texture the sphere, we need to create a new shader resource view, which we call smrv for 
// skymap resource view. You can see we have two new render states. one is a rasterizer state, which we will use to 
// disable backface culling, since when we create our sphere some triangles are facing the opposite way as the others.
// And a Depth/Stencil state, we will use to make sure our sky is ALWAYS behind all other geometry in the world. We 
// have two new integers, one for the vertices of our sphere, and the other for the faces. And last we have a new 
// matrix which defines our spheres world space.
void CreateSphere(int LatLines, int LongLines)
{
...
bool InitScene()
{
	...
	hr = D3DCompileFromFile(L"shaders.hlsl", 0, 0, "VS", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &VS_Buffer, 0);
	hr = D3DCompileFromFile(L"shaders.hlsl", 0, 0, "PS", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &PS_Buffer, 0);
	hr = D3DCompileFromFile(L"shaders.hlsl", 0, 0, "D2D_PS", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &D2D_PS_Buffer, 0);

	///////////////**************new**************////////////////////
	// The Skymap's VS and PS
	// Next, we need to create our new shaders.
	hr = D3DCompileFromFile(L"shaders.hlsl", 0, 0, "SKYMAP_VS", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &SKYMAP_VS_Buffer, 0);
 	hr = D3DCompileFromFile(L"shaders.hlsl", 0, 0, "SKYMAP_PS", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &SKYMAP_PS_Buffer, 0);
	///////////////**************new**************////////////////////
...
//	hr = D3DX11CreateShaderResourceViewFromFile( d3d11Device, L"grass.dds",	NULL, NULL, &CubesTexture, NULL );
...
//		ID3D11ShaderResourceView* SMTexture;
//	hr = D3DX11CreateTextureFromFile(d3d11Device, L"skymap.dds", &loadSMInfo, 0, (ID3D11Resource**)&SMTexture, 0);
	hr = CreateDDSTextureFromFileEx(d3d11Device, L"skymap.dds", 0, D3D11_USAGE_IMMUTABLE, D3D11_BIND_SHADER_RESOURCE,
...
// To keep up the skybox illusion, we need to make sure that no matter how far in any direction the camera moves, 
// we will never get any closer to the skybox, or else the illusion will not work.We will do this by making sure the 
// skybox sphere is centered around the camera every frame.Another thing is it doesn't actualy matter what the size of
// the skybox is. You can try changing the size of the skybox, but it will always appear the same no matter how big or
// small it is.
// To give the sky a slightly "flatter" look, you might want to keep the y value slightly less than the x and z values
// when scaling the sphere, so its kind of like a "squashed" sphere.
void UpdateScene(double time)
{
	//Reset cube1World
